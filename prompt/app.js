...
const metaKeys=[
{ id:1, category:"자동 개선", command:"/autoprompt", title:"짧은 요청을 실행용 프롬프트로 재작성", summary:"목표·대상·맥락·자료·제약·출력 형식을 보완해 바로 실행할 수 있는 프롬프트로 바꿉니다.", usage:"아이디어나 질문을 짧게 던졌지만 결과물의 품질을 높이고 싶을 때", instruction:"내 요청을 바로 실행하지 말고 목표·대상·맥락·자료·제약·출력 형식을 보완한 실행용 프롬프트로 먼저 재작성해. 결과를 크게 바꾸는 정보가 부족하면 질문을 최대 3개만 하고, 그렇지 않으면 가정을 표시한 뒤 진행해. 최종 프롬프트와 적용한 가정을 함께 제시해.", example:"'전통시장 활성화 방안 조사해줘' 뒤에 붙여 조사 범위·출처·산출물까지 구체화" },
{ id:2, category:"분석·계획·실행", command:"/APX", title:"분석→계획→실행 프레임", summary:"요구사항과 리스크를 분석하고, 실행계획과 평가기준을 세운 뒤 결과물을 완성합니다.", usage:"중요한 기획서·보고서·사이트·앱 작업처럼 중간 누락을 줄여야 할 때", instruction:"요청을 Analyze(목표·조건·누락·리스크 분석)→Plan(단계·담당 산출물·평가기준 계획)→eXecute(계획에 따른 실행) 순서로 처리해. 실행 전 계획을 간단히 보여주고, 실행 후 요구사항 체크리스트·가정·누락·다음 조치를 제시해.", example:"사업기획서 작성 요청 뒤에 붙여 기획 논리와 실행계획을 함께 검토" },
{ id:3, category:"검증·비판", command:"/REDTEAM", title:"반대 관점의 허점·실패 검토", summary:"완성본을 비판적 검토자 관점에서 점검해 오류와 실패 가능성을 찾고 수정안을 제시합니다.", usage:"틀리면 안 되는 보고서·정책·재무·보안·배포 전 결과물 검토", instruction:"완성본을 독립적인 Red Team 검토자로 점검해. 사실·논리·수치·출처·법적·보안·사용성 리스크를 분리하고, 발생 가능성이 높은 실패 시나리오를 최소 5개 제시해. 각 문제에 심각도·근거·수정안을 붙이고, 공격이나 우회가 아니라 방어와 개선에 집중해.", example:"제출 전 정책기획서의 근거 부족·예산 오류·실행 리스크를 냉정하게 점검" },
{ id:4, category:"완성·검증 조합", command:"/APX /REDTEAM", title:"분석·실행 후 독립 검증", summary:"APX로 결과물을 만든 다음 REDTEAM으로 검증하고 수정한 최종본까지 제공합니다.", usage:"공개·제출·배포하는 최종 산출물의 완성도를 높여야 할 때", instruction:"먼저 /APX 방식으로 요청을 분석하고 계획한 뒤 결과물을 작성해. 이어서 /REDTEAM 방식으로 사실·논리·수치·출처·법적·보안·사용성·요구사항 누락을 독립 검토하고, 발견한 문제를 직접 수정해 최종본을 제시해. 마지막에 변경 요약과 남은 리스크를 구분해.", example:"웹사이트 오픈 전 기획·코드·콘텐츠·접근성·SEO를 한 번에 검수" },
{ id:5, category:"사실·출처", command:"/FACTCHECK", title:"사실·날짜·출처 검증", summary:"주장과 수치를 사실·추정·의견으로 구분하고 1차 출처와 기준일을 확인합니다.", usage:"정책·시장·뉴스·보고서처럼 최신성과 정확성이 중요한 작업", instruction:"답변의 주장과 수치를 사실·추정·의견으로 구분해. 가능한 한 공식기관·원문·통계·학술자료를 우선 확인하고 출처 링크·발표일·기준일·불확실성을 표시해. 확인하지 못한 내용은 단정하지 말고 확인 필요로 표시해.", example:"최근 1년 상권정책 동향이나 시장규모를 조사할 때 사용" },
{ id:6, category:"근거·인용", command:"/SOURCES", title:"근거와 인용 구조화", summary:"핵심 주장마다 근거와 링크를 연결해 읽는 사람이 검증할 수 있게 합니다.", usage:"보고서·기사·강의자료·공모사업 기획서에 근거를 붙일 때", instruction:"각 핵심 주장 뒤에 근거 출처를 연결하고, 출처 유형·제목·기관·발표일·링크를 표로 정리해. 원문이 확인되지 않은 인용은 만들지 말고, 자료 간 불일치가 있으면 차이를 설명해.", example:"정책 제안서의 성과 수치와 제도 근거를 출처표와 함께 정리" },
{ id:7, category:"품질·테스트", command:"/QA", title:"품질검수와 테스트 케이스", summary:"결과물을 요구사항·기능·콘텐츠·접근성·예외상황 기준으로 테스트합니다.", usage:"웹·앱·대시보드·문서·강의자료를 공개하기 직전", instruction:"결과물을 QA 담당자처럼 점검해. 요구사항 추적표, 정상·경계·오류 테스트 케이스, 재현 절차, 기대 결과, 실제 결과, 심각도, 수정 우선순위를 표로 제시해. 확인하지 못한 항목은 미검증으로 구분해.", example:"모바일 메뉴·로그인·검색·저장·다운로드·404를 공개 전 점검" },
{ id:8, category:"보안·개인정보", command:"/SECURITY", title:"보안·개인정보 위험 점검", summary:"민감정보 노출, 인증·권한, 입력값, 외부 연동과 운영상 보안 위험을 검토합니다.", usage:"관리자 대시보드·회원·결제·파일 업로드·API가 있는 서비스", instruction:"보안 검토자로서 인증·권한·세션·입력값 검증·비밀키·로그·파일 업로드·외부 API·개인정보 보관·삭제·전송 위험을 점검해. 재현 가능한 방어적 수정안과 우선순위를 제시하고 비밀값은 출력하지 마.", example:"관리자 로그인과 결제 기능을 공개하기 전 보안 체크리스트 작성" },
{ id:9, category:"사용자·UX", command:"/UXCHECK", title:"사용자 여정과 접근성 점검", summary:"초보자와 모바일 사용자가 헤매지 않도록 화면 흐름·문구·접근성을 개선합니다.", usage:"홈페이지·앱·대시보드의 메뉴와 전환을 검수할 때", instruction:"초보 사용자와 모바일 사용자의 입장에서 첫 방문→탐색→입력→완료→재방문 여정을 따라가며 혼란 지점과 이탈 원인을 찾아. 정보 위계·버튼 문구·오류 안내·키보드·색 대비·스크린리더·터치 영역 개선안을 제시해.", example:"프롬프트 검색부터 복사·실행까지 세 단계로 줄이는 UX 검토" },
{ id:10, category:"초보자 설명", command:"/SIMPLE", title:"초보자용 쉬운 설명", summary:"전문용어를 풀고 단계별 예시와 확인 질문으로 처음 배우는 사람도 따라오게 합니다.", usage:"교육자료·매뉴얼·정책 안내·AI 사용법을 쉽게 설명할 때", instruction:"처음 배우는 사람을 대상으로 설명해. 전문용어는 짧게 풀어 쓰고, 핵심을 한 문장으로 먼저 제시한 뒤 단계·예시·주의사항·스스로 확인할 질문 순서로 구성해. 불필요한 배경 설명과 과도한 용어를 줄여.", example:"시니어 대상 ChatGPT 사용법이나 관리자 매뉴얼 작성" },
{ id:11, category:"실행·운영", command:"/ACTION", title:"실행계획과 담당자 지정", summary:"아이디어를 오늘 할 일·담당자·기한·완료 기준이 있는 실행계획으로 바꿉니다.", usage:"기획 결과를 실제 업무와 프로젝트 일정으로 옮길 때", instruction:"제안 내용을 실행 가능한 작업으로 분해해. 각 작업에 목적·담당 역할·시작일·기한·선행조건·완료 기준·필요자료·리스크·다음 행동을 표로 정리하고, 7일·30일·90일 우선순위를 제시해.", example:"컨설팅 결과를 90일 실행 로드맵과 KPI로 전환" },
{ id:12, category:"결정·비교", command:"/DECIDE", title:"대안 비교와 의사결정", summary:"대안을 기준별로 비교하고 추천 조건과 보류 조건을 명확히 합니다.", usage:"도구·플랫폼·사업모델·기술·예산안을 선택할 때", instruction:"대안들을 동일한 평가기준과 가중치로 비교해. 비용·효과·리스크·실행난이도·확장성을 표로 점수화하고, 어떤 조건에서 어느 대안을 선택할지와 추가로 확인할 정보를 제시해. 근거 없는 확정 추천은 피하고 판단의 전제를 표시해.", example:"GitHub Pages와 다른 호스팅, 두 가지 앱 기능 우선순위 비교" }
];
...
const selected=new Set(),metaSelected=new Set(); let category='전체',metaCategory='전체',workCategory='전체',recipeCategory='전체';
...
function renderMetaKeys(){const q=norm(el('metaSearch').value);const data=metaKeys.filter(x=>(metaCategory==='전체'||x.category===metaCategory)&&norm(x.category+x.command+x.title+x.summary+x.usage+x.instruction+x.example).includes(q));el('metaGrid').innerHTML=data.map(metaKeyCard).join('');el('metaEmpty').style.display=data.length?'none':'block';const vals=[...metaSelected];el('metaComboBar').classList.toggle('empty',!vals.length);el('metaComboBar').querySelector('span').textContent=vals.length?vals.join(' '):'선택한 작업 치트키가 없습니다.';el('metaComboCount').textContent=vals.length;if(el('metaResult'))el('metaResult').textContent=`${data.length}개 표시 · 전체 ${metaKeys.length}개`}
function metaKeyCard(x){const command=escapePromptText(x.command),instruction=escapePromptText(x.instruction),copy=escapePromptText(x.command+'\\n\\n'+x.instruction);return '<article class="meta-key-card '+(metaSelected.has(x.command)?'selected':'')+'"><div class="meta-key-head"><div><span class="badge">'+escapePromptText(x.category)+'</span><code>'+command+'</code></div><span>#'+String(x.id).padStart(2,'0')+'</span></div><h3>'+escapePromptText(x.title)+'</h3><p class="meta-key-summary">'+escapePromptText(x.summary)+'</p><div class="meta-key-usage"><b>사용 상황</b><span>'+escapePromptText(x.usage)+'</span></div><div class="meta-key-instruction"><b>확장 지침</b><span>'+instruction+'</span></div><p class="meta-key-example"><b>실전 예시</b> '+escapePromptText(x.example)+'</p><button class="copy" data-copy="'+copy+'">확장 지침 복사</button></article>'}
...
filters('categoryFilters',commands,'category',v=>{category=v;renderCommands()});filters('workFilters',works,'category',v=>{workCategory=v;renderList(works,workCategory,'workSearch','workGrid','workEmpty')});filters('recipeFilters',recipes,'category',v=>{recipeCategory=v;renderList(recipes,recipeCategory,'recipeSearch','recipeGrid','recipeEmpty')});filters('metaFilters',metaKeys,'category',v=>{metaCategory=v;renderMetaKeys()});
el('commandSearch').oninput=renderCommands;el('workSearch').oninput=()=>renderList(works,workCategory,'workSearch','workGrid','workEmpty');el('recipeSearch').oninput=()=>renderList(recipes,recipeCategory,'recipeSearch','recipeGrid','recipeEmpty');el('metaSearch').oninput=renderMetaKeys;
el('commandGrid').onclick=e=>{const b=e.target.closest('.key-card');if(!b)return;selected.has(b.dataset.command)?selected.delete(b.dataset.command):selected.add(b.dataset.command);renderCommands()};el('metaGrid').onclick=e=>{const b=e.target.closest('.meta-key-card');if(!b)return;const command=b.querySelector('code')?.textContent||'';if(metaSelected.has(command))metaSelected.delete(command);else metaSelected.add(command);renderMetaKeys()};
...
function activateView(name,scrollTop){
  ...
  document.title=name==="home"?"AI위드스쿨 프롬프트랩":(viewTitles[name]||"AI위드스쿨 프롬프트랩")+" · AI위드스쿨 프롬프트랩";
