...
if(homeLink)homeLink.addEventListener("click",e=>{e.preventDefault();activateView("home",true);history.replaceState(null,"","#homeView")});document.querySelectorAll('.footer-links a[href^="#"]').forEach(link=>link.addEventListener("click",e=>{const targetId=link.getAttribute("href").slice(1);if(!el(targetId))return;e.preventDefault();const initialHash=(location.hash||"").replace(/^#/,"");const initialView=initialHash.endsWith("View")?initialHash.slice(0,-4):(viewTitles[initialHash]?initialHash:"home");activateView(viewTitles[initialView]?initialView:"home",false);setTimeout(()=>el(targetId).scrollIntoView({behavior:"smooth",block:"start"}),0)}));
...
(() => {
  ...
  ["commandSearch", "workSearch", "recipeSearch", "planningSearch", "websiteSearch", "appSearch", "shopSearch", "astraSearch"].forEach(function (id) {
    const input = el(id);
    if (!input) return;
    let timer = null;
