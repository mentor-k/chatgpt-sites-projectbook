...
function card(x){const guide=inputGuide(x.prompt),example=escapePromptText(scenarioExample(x));return '<article class="prompt-card"><div class="meta"><span class="badge">'+escapePromptText(x.category)+'</span><span>#'+String(x.id).padStart(2,"0")+'</span></div><h3>'+escapePromptText(x.title)+'</h3><div class="text">'+x.prompt+'</div><div class="prompt-inputs"><b>먼저 바꿀 입력값</b><span>'+guide+'</span></div><div class="prompt-example"><b>초보자 예시</b><span>'+example+'</span></div><p class="tip">활용 포인트 · '+x.tip+'</p><button class="copy" data-copy="'+x.prompt.replaceAll('"','&quot;')+'">복사 후 사용</button></article>'}
